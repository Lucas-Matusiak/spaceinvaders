package fr.unilim.iut.spaceinvaders.model;

public class Constante {
	public static final int ESPACEJEU_LONGUEUR = 400;
    public static final int ESPACEJEU_HAUTEUR = 225;
	   
	   public static final int VAISSEAU_LONGUEUR = 30;
	   public static final int VAISSEAU_HAUTEUR = 20;
	   public static final int VAISSEAU_VITESSE = 7;
	
	   public static final char MARQUE_FIN_LIGNE = '\n';
	   public static final char MARQUE_VIDE = '.';
	   public static final char MARQUE_VAISSEAU = 'V';
	   public static final char MARQUE_MISSILE = 'M';
	   public static final char MARQUE_ENVAHISSEUR = 'E';
	   
	   public static final int MISSILE_LONGUEUR = 30;
	   public static final int MISSILE_HAUTEUR = 20;
	   public static final int MISSILE_VITESSE = 10;
	   
	   public static final int ENVAHISSEUR_LONGUEUR = 30;
	   public static final int ENVAHISSEUR_HAUTEUR = 18;
	   public static final int ENVAHISSEUR_VITESSE = 8 ;
	   
	   public static final int LIMITE_HAUT_JEU = 0;
	   public static final int LIGNE_DES_ORDONNEES_NUM_8 = 160;
	   
}
